...
test("updates visitor count and trivia successfully", async () => {
  ...
  fetch.mockResponseOnce(JSON.stringify({ text: "10 is a prime number!" }), {
    status: 200,
  });
  ...
  await waitFor(() => {
    ...
    expect(visitorCountElement.textContent).toBe(
      "10 visitors - 10 is a prime number!",
    );
...
test("handles error when fetching visitor count", async () => {
  ...
  expect(errorMessageElement.textContent).toBe(
    "Error Message: Failed to fetch visitor count.",
  );
});

test("handles error when fetching trivia", async () => {
  fetch.mockResponseOnce(JSON.stringify({ visitCount: 5 }), { status: 200 });

  fetch.mockResponseOnce(JSON.stringify({ error: "Failed to fetch trivia" }), {
    status: 500,
  });
  ...
  await waitFor(() => {
    const visitorCountElement = document.getElementById("visitor-count");
    expect(visitorCountElement.textContent).toBe(
      "5 visitors - No trivia available",
    );
    const errorMessageElement = document.getElementById("error-message");
    expect(errorMessageElement.textContent).toBe(
      "Error Message: Failed to fetch trivia.",
    );
  });
});
